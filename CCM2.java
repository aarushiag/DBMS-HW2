...
class Transaction implements Runnable
{
	...
	{		
		...
		{
				 while( !db.hmap_flight.get(F).flight_lock.tryLock())
				 {
					 Thread.sleep(1000);
				 }
				 
				 while(!db.hmap_passenger.get(id).pass_lock.tryLock())
				 {
					 Thread.sleep(1000);
				 }
	...
	void Transfer(int F1, int F2, int i) throws InterruptedException
	{
		...
		if(flight.hmap_passengers.get(i)!=null)
		{
			if(F1<F2)
			{
				while(!flight.flight_lock.tryLock())
				{
					Thread.sleep(1000);
				}
				...
				while(!flight1.flight_lock.tryLock())
				{
					Thread.sleep(1000);
				}
